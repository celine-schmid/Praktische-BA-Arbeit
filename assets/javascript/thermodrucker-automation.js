...
function draw() {
    ...
    //Canvas
    //////////////////////////////////////////////////////
    // div in dem der Canvas platziert wird
    ...
    let wachstumsrate_3 = int(random(1,3))*int(sqrt((rasterhoehe*rasterhoehe)+(rasterbreite*rasterbreite)));
    let aeussersterPunkt = 2 * wachstumsrate_1 + 6 * wachstumsrate_2 + wachstumsrate_3 + 5 * buchstabenabstand + 19.1 * rasterbreite + 3 * unterschneidung;

    var canvas = createCanvas(aeussersterPunkt, canvasDiv.offsetHeight);
    // das anvas container div zum parent des Canvas machen
    ...
    //Buchstaben
    let y_MittelLinie = 6*rasterhoehe;
    let y_HLinie = y_MittelLinie-3.5*rasterhoehe; //+dynamischeRate       //2
    let y_GrundLinie = y_MittelLinie+3*rasterhoehe;                     //2
...
// Dynamically adjust parent div width based on canvas width
...
// Call adjustParentWidth() whenever the window is resized
window.addEventListener('resize', adjustParentWidth);